...
def get_icon_path() -> str | None:
    """获取图标路径。"""
    name_map = {"Windows": "FBeePet.ico", "Darwin": "FBeePet.icns"}
    icon = RESOURCES_DIR / name_map.get(platform.system(), "FBeePet.png")
...
def _set_windows_app_id() -> None:
    ...
    try:
        ...
        ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID("FBeePet.desktop.app")
    except Exception:
        pass
...
def main() -> None:
    ...
    if is_public_bind_host(host):
        logger.info(f"FBeePet Desktop 启动中: http://{client_host}:{port} (bind: {host})")
    else:
        logger.info(f"FBeePet Desktop 启动中: http://{client_host}:{port}")
    ...
    try:
        ...
        logger.info("FBeePet Desktop 启动成功")
    ...
    logger.info("FBeePet Desktop 已退出")
    os._exit(0)
