...
async def get_settings() -> SettingsResponse:
    ...
    try:
        ...
        # 构建 providers 响应（不脱敏，直接返回；进程内运行时 key 优先）
        providers_response = {}
        for name, provider_config in config.providers.items():
            effective_keys = list(get_runtime_api_keys(name))
            ...
            providers_response[name] = ProviderConfigResponse(
                ...
                api_key=effective_keys[0] if effective_keys else "",
    ...
    try:
        ...
        if request.providers:
            for name, provider_data in request.providers.items():
                ...
                # API Key 只存进程内存(网页端输入、不落盘,进程退出即失效)：
                # 磁盘配置里的 key 一律清空,避免把运行时 key 持久化
                if "api_keys" in provider_data:
                    ...
                    set_runtime_api_keys(name, keys)
                    provider_config.api_keys = []
                elif "api_key" in provider_data:
                    set_runtime_api_keys(name, [provider_data["api_key"]])
