...
from backend.modules.providers.runtime import get_runtime_api_keys
...
def resolve_session_runtime_config(app_config: AppConfig, session: Optional[Any]) -> SessionRuntimeConfig:
    ...
    if api_key is None:
        # 进程内运行时 key(网页端输入、不落盘)优先于磁盘配置
        runtime_keys = get_runtime_api_keys(provider_name)
        if runtime_keys:
            api_key = runtime_keys[0]
    if api_key is None:
    ...
    model_response["api_key"] = api_key or ""
