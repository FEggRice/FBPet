...
class Skill:
    ...
    def _apply_frontmatter(self, data: Dict[str, Any], metadata: Dict[str, Any]) -> None:
        ...
        if isinstance(nested, dict):
            FBeePet_meta = nested.get("FBeePet")
            if isinstance(FBeePet_meta, dict):
                if "requires" in FBeePet_meta:
                    # 顶层 requires 优先；仅在其缺省时回退到嵌套写法
                    nested_requires = _coerce_requires(FBeePet_meta.get("requires"))
                    if nested_requires and not metadata["requires"]:
                        metadata["requires"] = nested_requires
                if "always" in FBeePet_meta:
                    metadata["always"] = _coerce_bool(FBeePet_meta["always"])
