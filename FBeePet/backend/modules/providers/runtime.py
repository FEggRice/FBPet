...
# 进程内 API Key(网页端输入,不落盘):进程退出即失效,随桌宠退出一起清空。
# 优先级:会话级覆盖 > 运行时内存 key > 磁盘配置 key。
_RUNTIME_API_KEYS: dict[str, List[str]] = {}
_RUNTIME_API_KEYS_LOCK = threading.Lock()


def set_runtime_api_keys(provider_id: str, api_keys: List[str]) -> None:
    """设置某个 provider 的进程内 API Key 列表(空列表则清除)。"""
    normalized = [str(k).strip() for k in api_keys if str(k).strip()]
    with _RUNTIME_API_KEYS_LOCK:
        if normalized:
            _RUNTIME_API_KEYS[provider_id] = normalized
        else:
            _RUNTIME_API_KEYS.pop(provider_id, None)


def get_runtime_api_keys(provider_id: str) -> List[str]:
    """返回某个 provider 的进程内 API Key 列表(未设置则为空列表)。"""
    with _RUNTIME_API_KEYS_LOCK:
        return list(_RUNTIME_API_KEYS.get(provider_id, []))
...
def get_provider_runtime_state(
    ...
    *,
    ...
    # 无显式覆盖时,回退到进程内运行时 key(网页端输入、不落盘)
    runtime_keys = get_runtime_api_keys(provider_id)
    if not api_key_override and runtime_keys:
        api_key_override = runtime_keys[0]
