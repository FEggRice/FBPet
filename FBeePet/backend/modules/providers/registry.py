...
# FBeePet 只保留 DeepSeek / OpenAI / Ollama + 两个自定义接口
...
def get_all_providers() -> Dict[str, ProviderMetadata]:
    """获取所有 provider(FBeePet 只开放允许的 5 个)"""
    return {pid: PROVIDER_REGISTRY[pid] for pid in ALLOWED_PROVIDER_IDS}


def get_provider_ids() -> List[str]:
    """获取所有 provider ID(FBeePet 只开放允许的 5 个)"""
    return [pid for pid in PROVIDER_REGISTRY if pid in ALLOWED_PROVIDER_IDS]
